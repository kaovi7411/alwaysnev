...
exports.run = (client, message, args) => {
  ...
  let muteRole = client.guilds.get(message.guild.id).roles.find('name', 'Büyücü ✔️');
  if (!modlog) return message.reply('`💠┇mod-log` kanalını bulamıyorum.').catch(console.error);
  if (!muteRole) return message.reply('`Büyücü ✔️` adlı bir rol bulamıyorum.').catch(console.error);
